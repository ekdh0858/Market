...
//		//클라이언트가 어떤 방식으로 요청하더라도 service메서드가 호출됨
//		//클라이언트가 GET 방식으로 요청했다는건 컨텐츠를 전달박기만 위함이고
//		//클라이언트가 POST 방식으로 요청했다는건 컨텐츠를 생성하기 위함임
//		//이렇게 요청 방식에 따라서 클라이언트의 요청 사항이 다름
//		//service 메서드 하나만으로는 모든 요청 을 처리하기에는 너무 불편하니까
//		//실제로 개발할 떄는 service 메서드를 오버라이딩하지 않고
//		//사용자의 요청 방식에 맞는 처리를 할 수 있게 해주는 다른 메서드를 오버라이딩함
