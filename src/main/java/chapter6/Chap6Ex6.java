...
public class Chap6Ex6 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//식의 단축 연산 -> 논리 연산을 할때 굳이 할 팔요없는 연산은 하지 않는 현상
		if((userId != null && id.equals(userId)) && (userPw != null && pw.equals(userPw))) {
			...
			//setAttribute(name.value) -> session에 값을 저장할 수 있음
			session.setAttribute("isLogin", "true");
			session.setAttribute("userName", "김철수");
		}
		boolean isLogin = false;
		HttpSession session = request.getSession();
		//session.getAttribute(name) -> 세션에 저장된값을 불러올 수 없음
		//세션에 isLogin 키가 없을 경우 NullpoiintException이 발생할 수 있음		
